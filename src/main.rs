...
fn get_version_string() -> String {
    format!("{} v{}\nBuild: {}", PKG_NAME, PKG_VERSION, GIT_HASH)
}
...
fn set_gauge_if_valid(metric_name: &str, value: f64) -> bool {
    ...
    if value == -1.0 || value <= -1.7e308 {
        log(
            "DEBUG",
            &format!("Skipping invalid metric {}", metric_name),
            &[
...
async fn main() -> Result<()> {
    ...
    match setup_metrics(&args.bind_address, args.metrics_port) {
        Ok(_) => {
            log(
                "INFO",
                &format!("{} metrics endpoint started", PKG_NAME),
                &[
                    (
                        "endpoint",
                        format!("http://{}:{}/metrics", args.bind_address, args.metrics_port),
                    ),
        ...
        Err(e) => {
            log(
                "ERROR",
                &format!("Failed to start {} metrics endpoint", PKG_NAME),
                &[
                    ("error", format!("{}", e)),
    ...
    let client_path: zvariant::OwnedObjectPath = manager
        .call::<_, _, zvariant::OwnedObjectPath>("GetClient", &())
        .await?;

    log(
        "INFO",
        "Got client path",
        &[("path", format!("{}", client_path))],
    );
    ...
    client
        .set_property("DesktopId", &PKG_NAME.to_string())
        .await?;
    log(
        "INFO",
        "Set client desktop ID",
        &[("desktop_id", PKG_NAME.to_string())],
    );
    ...
    client
        .set_property("DistanceThreshold", &args.distance_threshold)
        .await?;
    log(
        "INFO",
        "Set distance threshold",
        &[("threshold_meters", args.distance_threshold.to_string())],
    );

    // Set time threshold (in seconds)
    client
        .set_property("TimeThreshold", &args.time_threshold)
        .await?;
    log(
        "INFO",
        "Set time threshold",
        &[("threshold_seconds", args.time_threshold.to_string())],
    );

    // Set requested accuracy level
    client
        .set_property("RequestedAccuracyLevel", &(accuracy_level as u32))
        .await?;
    log(
        "INFO",
        "Set accuracy level",
    ...
    tokio::spawn(async move {
        if let Err(e) = ctrl_c().await {
            log(
                "ERROR",
                "Failed to listen for ctrl+c signal",
                &[("error", format!("{}", e))],
            );
            return;
        }
        ...
        log(
            "INFO",
            "Shutdown signal received, stopping GeoClue2 client",
            &[],
        );
        ...
        .await
        {
            Ok(shutdown_client) => {
                ...
                if let Err(e) = shutdown_client.call::<_, _, ()>("Stop", &()).await {
                    log(
                        "ERROR",
                        "Failed to stop GeoClue2 client",
                        &[("error", format!("{}", e))],
                    );
                } else {
            ...
            Err(e) => {
                log(
                    "ERROR",
                    "Failed to create shutdown client proxy",
                    &[("error", format!("{}", e))],
                );
            }
        }
    ...
    while let Some(signal) = location_updated_stream.next().await {
        // Update counter whenever we get a new location
        {
            ...
            metrics::gauge!("geoclue_location_updates_received")
                .set(tracker.received_updates as f64);
